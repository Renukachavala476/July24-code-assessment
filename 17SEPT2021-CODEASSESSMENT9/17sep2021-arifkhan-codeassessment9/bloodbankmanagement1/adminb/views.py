...
def addAdmin(request):
    if (request.method=="POST"):
        ...
        admin_serialize=BloodAdSerializer(data=mydata)
...
def login_check(request):
    ...
    getadmin=BloodAd.objects.filter(username=username,password=password)
    admin_serializer=BloodAdSerializer(getadmin,many=True)
...
def viewall(request): 
    fetchdata=requests.get("http://127.0.0.1:8000/adminb/viewallapi/").json()
...
def update_data_read(request):
    if(request.method=="POST"):
        ...
        print(jsondata)
        ApiLink="http://127.0.0.1:8000/adminb/viewapi/" + getId
