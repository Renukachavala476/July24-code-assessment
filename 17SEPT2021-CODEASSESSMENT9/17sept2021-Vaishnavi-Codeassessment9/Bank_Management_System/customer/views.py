...
import requests
from bank.models import Customer, Donor,Admin
from bank.serializer import BankSerializer,CustomerSerializer
...
def loginCheck(request):
    try:
        ...
        getcustomer=Donor.objects.filter(username=getsusername,password=getspassword)
        pserialize=CustomerSerializer(getcustomer,many=True)
...
def update_search_api(request):
    try:
        ...
        getnumbers=Customer.objects.filter(username=getnumber)
...
def update_data_read(request):
    if(request.method=="POST"):
        ...
        print(jsondata)
        ApiLink="http://127.0.0.1:8000/customer/viewapi/" + getId
