...
def adminRegistration(request):
    if request.method=="POST":
        ...
        data=AdminRegm.objects.filter(eemail=username)
        if data:
            ...
            print(dict1)
            registereddata=AdminRegs(data=dict1)
...
def adminLogin(request):
    if request.method=="POST":
        ...
        getpassword=request.POST.get('epassword')
        flag=0
        data=Adminlogm.objects.filter(eusername=getusername,epassword=getpassword)
        print(data)
        if data:
            adminserialiser=Adminlogs(data,many=True)
