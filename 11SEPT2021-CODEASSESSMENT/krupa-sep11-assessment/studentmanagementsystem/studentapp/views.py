...
def login_check(request):
    try:
        ...
        getUsers = Student.objects.filter(username=getUsername, password=getPassword)
        user_serialiser = StudentSerializer(getUsers, many=True)
...
def profile(request):
    # if (request.method=="GET"):
        try:
            ...
            getUsers = Student.objects.get(id=getUid)
            user_serialiser = StudentSerializer(getUsers)
...
def user_create(request):
    if (request.method == "POST"):
        ...
        try:
            getUsername = request.POST.get("username")
            getPassword = request.POST.get("password")
            getUsers = Student.objects.filter(username=getUsername)
            user_serialiser = StudentSerializer(getUsers, many=True)
            print(user_serialiser.data)
            if (user_serialiser.data):
                ...
                user_serialize = StudentSerializer(data=request.POST)
